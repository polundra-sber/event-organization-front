...
import { EventCostAllocationPageContent } from "@/components/events/cost-allocation-page/EventCostAllocationPage";
...
export default function EventCostAllocationPage() {
  ...
  // Получаем метаданные пользователя для мероприятия (только при наличии validEventId)
  const {
    data: metadata,
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      // Проверяем, что eventId уже обработан
      setIsLoading(false);
    }
  }, [router, validEventId]);
  ...
  return <EventCostAllocationPageContent event_id={validEventId} />;
}
